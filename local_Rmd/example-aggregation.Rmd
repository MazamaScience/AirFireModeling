...
Bluesky air-quality models by default generate new models for each day. That means for each $N$ days we decide to obtain data for, there will be $N$ independent models generated. For example, if we wanted to preform an analysis on _two_ days of data we would obtain a total of _two_ seperate models, with `chunks` produced - each model looking $N$ days in advanced. Aggregation allows us to aggregate the first $n$ chunks for each day, so we can use the most accurate (less time advance) models for multi-day analysis.
