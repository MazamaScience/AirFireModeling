#' @keywords bs_grid
#' @export
#' @title Subset a \code{bs_grid} Data List with a Grid Mask 
...
#' @description This function subsets the \code{bs_grid} data list and 
...
grid_subsetByMask <- function(bs_grid, gridMask, countryCodes=c('CA','US')) {
  ...
  # Sanity check
  if ( length(bs_grid$longitude) != nrow(gridMask) || length(bs_grid$latitude) != ncol(gridMask) ) {
  ...
  lon2D <- matrix(allLons, nrow=length(bs_grid$longitude), ncol=length(bs_grid$latitude), byrow=FALSE)
  lat2D <- matrix(allLats, nrow=length(bs_grid$longitude), ncol=length(bs_grid$latitude), byrow=FALSE)
  ...
  pm25Matrix <- matrix(unraveledPM25, nrow=length(bs_grid$time), ncol=cellCount, byrow=TRUE)
  ...
  for (name in names(bs_grid)) {
    # NOTE:  bs_grid$time has class = c("POSIXct","POSIXt") so we only compare the first one
