...
gridMap <- function(bs_grid, param="pm25",
                    xlim=range(bs_grid$longitude), ylim=range(bs_grid$latitude), 
                    tMask=rep(TRUE, times=length(bs_grid$time)), slice=get('max'),
  ...
  if ( is.null(main) ) {
    title(main=paste0(bs_grid$model, "\n", timeRange[1], " to ",timeRange[2], " UTC"))
  } else {
    title(main=main)
  }
